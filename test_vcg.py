...
from vcg.model_utils import create_model
from vcg.dataset import create_loader
from vcg.utils import save_skeleton
...
def main():
    with torch.no_grad():
        ...
        pretrain = test_config.get('pretrain', False)
        ## For reproducibility
        ...
        if is_supervised:
            ...
            logger.info('Conditional model, we will use label as condition.')
        ...
        if model_path:
            ...
            logger.info('Non-trained model is loaded.')
        ...
        if loader_config is not None:
            ...
            for res in custom_save_results:
                ...
                custom_res_dirs.append(res.get('dir', None))
            ...
            for t in tqdm(data_loader, desc="Predicting"):
                ...
                res = forward_pass(model, x, y, c, pretrain = pretrain)
                if verbose:
                ...
                if len(results['input']) < eval_batch_num:
                    append_results(results=results, x = x, y = y, c = c,
                                            path = path, res = res, data_form = data_form,
            ...
            if eval_metrics is not None:
                ...
                eval_res = evaluate_results(results, evaluators, data_form = data_form, verbose = True)
                if len(eval_res) > 0:
            ...
            for res_name, res_dir in zip(custom_res_names, custom_res_dirs):
                ...
                # print(res_dir, results.keys())
                if res_dir is not None and len(results[res_name]) > 0:
                    logger.info(f'Saving {res_name} results to {res_dir} ...')
                    if not os.path.exists(res_dir):
                        os.makedirs(res_dir)
                    if res_name == 'radius':
                        for idx, (sk, r) in enumerate(zip(results['recon_skeleton'], results['radius']) ):
                            origin_path = results['path'][idx]
                            if origin_path != '':
                                filename = os.path.basename(origin_path).split('.')[0]
                            else:
                                filename = 'sample_{:03d}'.format(idx)
                            output_path = os.path.join(res_dir, filename + '.npy')
                            save_skeleton(sk, r, output_path)    
                    else:
                        for idx, res_data in enumerate(results[res_name]):
                            ...
                            save_data(res_data, data_form=data_form, output_path=output_path)
            ...
            if gen_dir is not None:
                ...
                if inter_config is not None:
                    ...
                    for gid in range(group_num):
                        ...
                        if type(x_bar) == tuple:
                            for iid, (_sk, _r) in enumerate(zip(*x_bar)):
                                # x_bar_np = _x_bar.cpu().detach().numpy()
                                _sk_np = _sk.cpu().detach().numpy()
                                _r_np = _r.cpu().detach().numpy()
                                output_path = os.path.join(gen_dir, 'gen_inter_group{:02d}_{:02d}.npy'.format(gid, iid))
                                save_skeleton(_sk_np, _r_np, output_path)
                            x_bar = x_bar[0]
                        for iid, _x_bar in enumerate(x_bar):
                            x_bar_np = _x_bar.cpu().detach().numpy()
                            output_path = os.path.join(gen_dir, 'gen_inter_group{:02d}_{:02d}'.format(gid, iid))
                            save_data(x_bar_np, data_form, output_path)
                ...
                if random_sample_num > 0:
                    ...
                    if type(x_bar) == tuple:
                        for iid, (_sk, _r) in enumerate(zip(*x_bar)):
                            ...
                            _sk_np = _sk.cpu().detach().numpy()
                            _r_np = _r.cpu().detach().numpy()
                            output_path = os.path.join(gen_dir, 'gen_rand_{:02d}.npy'.format(iid))
                            save_skeleton(_sk_np, _r_np, output_path)
