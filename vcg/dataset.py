...
class PcdSDFDataset(PcdDataset):
    def __init__(self, data_path, data_transform = None, 
                ...
                target_suffix='.npy', resolution = 0.01, **kwargs):
    ...
    def __getitem__(self, index):
        ...
        if self.data_transform:
            ...
            sdf = self.target_transform((sdf, self.coord))

            set_random_state(n_state, t_state)
            coord = self.target_transform(self.coord)
            # save_pcd(pcd_path+'.transformed.ply', pcd.numpy())
        return pcd, coord, sdf[..., None], self.pcd_path_list[index]
...
class PcdSDFWithClassLabelDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.data_transform:
            ...
            # save_pcd(pcd_path+'.transformed.ply', pcd.numpy())
        return pcd, cls_label, coord, sdf[..., None], self.pcd_path_list[index]
