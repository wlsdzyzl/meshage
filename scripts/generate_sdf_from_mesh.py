...
def generate_sdf(surface, normals, resolution=0.01):
    ...
    # dir_vec = (grid_points - closest_surface) / LA.norm(grid_points - closest_surface, 
...
def process(mesh_path, sdf_path, recon_mesh_path, resolution=0.01, 
    ...
    try:
        ...
        logger.error(f"skipping broken mesh: {mesh_path}")
        logger.error(e)
        return
    if len(mesh.vertices) == 0:
        logger.info(f"skipping empty mesh: {mesh_path}")
        return
    ...
    if not skip:
        ...
        save_ply(mesh_path+'.ply', surface, faces = mesh.faces)
...
if __name__ == "__main__":
    ...
    for mesh_path in mesh_path_list:
        ...
        if skip: logger.info(f'Skip processed file {meth_path}, only record the water-tight attribute.')
